...
class Ocam_Updator
{
    ...
    public static function createFullTagAndEndPointDatabase()
    {
        ...
        require_once(dirname(__FILE__) . '/class-ocam-rewrite-rule-validator.php');
        ...
        if (null !== $savedToken || '' !== $savedToken->access_token) {
            if (self::isRewriteRouteEnabled()) {
                $endPointData = Ocam_Get::execute("https://www.adback.co/api/end-point/me?access_token=" . $savedToken->access_token);
                ...
                for ($i = 0; $i < 5; $i++) {
                    ...
                    $endPointData = Ocam_Get::execute("https://www.adback.co/api/end-point/refresh?access_token=" . $savedToken->access_token);
                    $endPoints = json_decode($endPointData, true);
                }
            }

            $fullScriptData = Ocam_Get::execute("https://www.adback.co/api/script/me/full?access_token=" . $savedToken->access_token);
